...
public partial class LineBookingToolCA : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        //SqlCommand command = new SqlCommand("SELECT Username FROM [View_FormNameByUser] WHERE Username = '" + Session["Username"] + "'and (UsrGroup='1') and frmnam25 = 'LineBookingTool.aspx'  ", conn);
    ...
    private void LoadResources()
    {
        ...
        SqlConnection conn = new SqlConnection(DbConnect.x);
        SqlDataAdapter da = new SqlDataAdapter("select distinct line_no as line from Line_Booking where ord_factory like 'CA%' order by line_no", conn);
    ...
    private DataTable DbGetEvents(DateTime start, int days)
    {
        ...
        //----- for tooltip-------
        string qry = @"select l.line_no as Line,l.po_no as PoNo,l.ord_quantity as ordQty,isnull(l.targetQty*l.planDay,0) as lineQty,isnull(p.ProdQty,0) as ProdQty,l.targetQty,l.planDay-isnull(p.prodDay,0) as remDay
			from (select l.line_no,l.ord_no,l.po_no,sum(SumDay) as planDay,ord_quantity,isnull(target_dy,0) as targetQty 
				from sanpro.dbo.Line_Booking l where l.ord_factory like 'CA%' group by l.line_no,l.ord_no,l.po_no,ord_quantity,target_dy) l left join
			...
			on l.ord_no=p.aStyle and l.po_no=p.aPO and l.line_no=p.aLine order by l.line_no";
    ...
    {
        if (Session["Username"].ToString() == "shakil" || Session["Username"].ToString() == "prince" || Session["Username"].ToString() == "Jayanta" || Session["Username"].ToString() == "moshiur")
        {
            string id = e.Value;
